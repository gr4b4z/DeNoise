...
/// </summary>
public sealed record AlertHubPrincipal(
    ...
    IReadOnlySet<string> Permissions, bool MustChangePassword, CredentialKind Credential, string? SessionId = null, Guid? TokenId = null)
{
    ...
    public static AlertHubPrincipal ForSession(User user, string sessionId) => new(
    ...
    public static AlertHubPrincipal ForToken(User user, PersonalAccessToken token)
    {
        ...
        return new AlertHubPrincipal(user.UserId, user.Username, user.DisplayName, user.RoleNames, user.Scopes, effective, user.MustChangePassword, CredentialKind.PersonalAccessToken, TokenId: token.TokenId);
    }
}
