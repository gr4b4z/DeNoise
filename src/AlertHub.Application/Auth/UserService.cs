...
{
    public async Task<(User User, string TemporaryPassword)> CreateAsync(CreateUser request, AlertHubPrincipal actor, string correlationId, CancellationToken ct = default)
    {
    ...
    public async Task<User> UpdateAsync(Guid userId, UpdateUser request, AlertHubPrincipal actor, string correlationId, CancellationToken ct = default)
    {
        Require(actor, Permissions.UserManage);
    ...
    public async Task<string> ResetPasswordAsync(Guid userId, AlertHubPrincipal actor, string correlationId, CancellationToken ct = default)
    {
        Require(actor, Permissions.UserManage);
        var user = await users.GetAsync(userId, ct) ?? throw new KeyNotFoundException("user not found");
        var temporary = PasswordPolicy.GenerateTemporary();
    ...
    public async Task SetDisabledAsync(Guid userId, bool disabled, AlertHubPrincipal actor, string correlationId, CancellationToken ct = default)
    {
        Require(actor, Permissions.UserManage);
        var user = await users.GetAsync(userId, ct) ?? throw new KeyNotFoundException("user not found");
    ...
    public async Task UnlockAsync(Guid userId, AlertHubPrincipal actor, string correlationId, CancellationToken ct = default)
    {
        Require(actor, Permissions.UserManage);
        var user = await users.GetAsync(userId, ct) ?? throw new KeyNotFoundException("user not found");
        user.LockedUntil = null;
    ...
    private static void Require(AlertHubPrincipal actor, string permission)
    {
    ...
    private void Audit(AlertHubPrincipal actor, string action, User target, string correlationId, object? after, object? before = null)
...
public sealed class PersonalAccessTokenService(IPersonalAccessTokenRepository tokens, IUserRepository users, ISecretHasher hasher, IAuditWriter audit, IUnitOfWork uow, TimeProvider time)
{
    ...
    public async Task<(PersonalAccessToken Token, string Plaintext)> CreateAsync(AlertHubPrincipal actor, string name, string[] scopes, DateTimeOffset? expiresAt, string correlationId, CancellationToken ct = default)
    {
    ...
    public async Task<bool> RevokeAsync(AlertHubPrincipal actor, Guid tokenId, string correlationId, CancellationToken ct = default)
    {
    ...
    public async Task<AlertHubPrincipal?> AuthenticateAsync(string bearer, CancellationToken ct = default)
    {
        ...
        return AlertHubPrincipal.ForToken(user, token);
    }
}
