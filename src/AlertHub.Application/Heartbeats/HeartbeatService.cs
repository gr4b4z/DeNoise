    ...
    ISecretHasher hasher, IAuditWriter audit, IUnitOfWork uow, IOptions<HeartbeatOptions> options, TimeProvider time)
{
    ...
    public async Task<HeartbeatCreated> CreateAsync(HeartbeatDefinition definition, AlertHubPrincipal actor, string correlationId, CancellationToken ct = default)
    {
    ...
    public async Task<Heartbeat> UpdateAsync(Guid id, int expectedVersion, HeartbeatDefinition definition, AlertHubPrincipal actor, string correlationId, CancellationToken ct = default)
    {
    ...
    public async Task DeleteAsync(Guid id, int expectedVersion, AlertHubPrincipal actor, string correlationId, CancellationToken ct = default)
    {
        var hb = await RequireAsync(id, actor, ct);
        if (hb.Version != expectedVersion) throw new Episodes.VersionConflictException(id, hb.Version);
    ...
    public async Task<Heartbeat> PauseAsync(Guid id, int expectedVersion, string reason, AlertHubPrincipal actor, string correlationId, CancellationToken ct = default)
    {
    ...
    public async Task<Heartbeat> ResumeAsync(Guid id, int expectedVersion, AlertHubPrincipal actor, string correlationId, CancellationToken ct = default)
    {
        var hb = await RequireAsync(id, actor, ct);
        if (hb.Version != expectedVersion) throw new Episodes.VersionConflictException(id, hb.Version);
        var outcome = await processing.RunAsync(async session =>
    ...
    public async Task<HeartbeatCreated> RotateTokenAsync(Guid id, int expectedVersion, AlertHubPrincipal actor, string correlationId, CancellationToken ct = default)
    {
        var hb = await RequireAsync(id, actor, ct);
        if (hb.Version != expectedVersion) throw new Episodes.VersionConflictException(id, hb.Version);
        var now = time.GetUtcNow();
    ...
    public async Task<string> ExportAsync(Guid? teamId, AlertHubPrincipal actor, CancellationToken ct = default)
    {
    ...
    public async Task<HeartbeatImportResult> ImportAsync(string yaml, bool dryRun, AlertHubPrincipal actor, string correlationId, CancellationToken ct = default)
    {
    ...
    private async Task<Heartbeat> RequireAsync(Guid id, AlertHubPrincipal actor, CancellationToken ct)
    {
    ...
    private async Task EnsureSystemIntegrationAsync(CancellationToken ct)
    {
        ...
        integrations.Add(new Integration
        {
            ...
            Name = "alert-hub-heartbeats",
            Type = IntegrationTypes.GenericWebhook,
            AccessScope = "alert-hub",
    ...
    private void Audit(AlertHubPrincipal actor, string action, Heartbeat hb, string correlationId, object? before, object? after)
        => audit.Record(AuditEntry(actor, action, hb, correlationId, before, after, time.GetUtcNow()));

    private static AuditEntry AuditEntry(AlertHubPrincipal actor, string action, Heartbeat hb, string correlationId, object? before, object? after, DateTimeOffset now) => new()
    {
