...
{
    Task<PagedResponse<EpisodeListItem>> ListAsync(AlertHubPrincipal principal, EpisodeFilter filter, IReadOnlyCollection<Guid> myTeamIds, CancellationToken ct = default);
    Task<EpisodeDetail?> GetAsync(AlertHubPrincipal principal, Guid episodeId, CancellationToken ct = default);
    Task<PagedResponse<TimelineEntry>> TimelineAsync(AlertHubPrincipal principal, Guid episodeId, int limit, string? cursor, string? kind, CancellationToken ct = default);
    Task<RelatedEpisodes?> RelatedAsync(AlertHubPrincipal principal, Guid episodeId, CancellationToken ct = default);
    Task<(byte[] Body, string? ContentType, DateTimeOffset ReceivedAt)?> RawPayloadAsync(AlertHubPrincipal principal, Guid episodeId, Guid eventId, CancellationToken ct = default);
    Task<IReadOnlyDictionary<string, int>> ViewCountsAsync(AlertHubPrincipal principal, IReadOnlyCollection<Guid> myTeamIds, CancellationToken ct = default);
    Task<TeamOverview?> TeamOverviewAsync(AlertHubPrincipal principal, Guid teamId, CancellationToken ct = default);
}
