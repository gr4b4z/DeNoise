...
/// <summary>
/// Feeds <c>alerthub_job_queue_depth{kind}</c> and <c>alerthub_job_oldest_age_seconds{kind}</c> (ADR-9) from a
/// periodic aggregate query; observable gauges read the last snapshot so metric collection never touches the DB.
/// </summary>
public sealed class QueueGauges(IServiceScopeFactory scopes, AlertHubMetrics metrics, TimeProvider time, ILogger<QueueGauges> logger) : BackgroundService
{
