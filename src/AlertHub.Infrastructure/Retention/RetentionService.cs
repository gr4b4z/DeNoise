...
namespace AlertHub.Infrastructure.Retention;
...
/// </summary>
public sealed class RetentionService(AlertHubDbContext db, RawEventPartitions partitions, IOptions<RetentionOptions> options, IOptions<Application.Heartbeats.HeartbeatOptions> heartbeats, TimeProvider time, ILogger<RetentionService> logger) : IRetentionRunner
{
    ...
    private AuditEntry Entry(DateTimeOffset now, string action, string targetType, string targetId, string correlation, string? after) => new()
    {
        ...
        ActorId = "retention",
        ActorDisplay = "Alert Hub retention",
        Action = action,
