...
{
    public const string HttpClientName = "alerthub-webhook";
    ...
    public async Task<ChannelResult> SendAsync(ResolvedDestination destination, OutboxMessage message, string body, CancellationToken ct)
    {
        ...
        request.Headers.TryAddWithoutValidation("X-AlertHub-Event", message.Type);
        request.Headers.TryAddWithoutValidation("X-AlertHub-Timestamp", timestamp);
        if (destination.SigningSecret is { Length: > 0 } secret)
        {
            request.Headers.TryAddWithoutValidation("X-AlertHub-Signature", "v1=" + Sign(secret, timestamp, body));
        }
