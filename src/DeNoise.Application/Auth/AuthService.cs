    ...
    IAuditWriter audit, IUnitOfWork uow, TimeProvider time, IOptions<LocalAuthOptions> options)
{
    ...
    public async Task<IReadOnlyList<string>> ChangePasswordAsync(DeNoisePrincipal principal, string current, string next, string correlationId, System.Net.IPAddress? ip, CancellationToken ct = default)
    {
