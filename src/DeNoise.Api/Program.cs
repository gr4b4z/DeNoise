...
builder.Services.AddAuthentication(DeNoiseAuthenticationHandler.SchemeName)
    .AddScheme<AuthenticationSchemeOptions, DeNoiseAuthenticationHandler>(DeNoiseAuthenticationHandler.SchemeName, null);
...
builder.Services.AddRateLimiter(o =>
{
    ...
    o.GlobalLimiter = PartitionedRateLimiter.Create<HttpContext, string>(ctx =>
    {
        ...
        var key = ctx.Request.Cookies[DeNoiseAuthenticationHandler.CookieName] ?? ctx.Request.Headers.Authorization.ToString();
...
builder.Services.AddOpenApi("v1", o =>
{
    ...
    {
        doc.Info.Title = "DeNoise application API";
        doc.Info.Version = "v1";
        ...
        doc.Components.SecuritySchemes["session"] = new OpenApiSecurityScheme { Type = SecuritySchemeType.ApiKey, In = ParameterLocation.Cookie, Name = DeNoiseAuthenticationHandler.CookieName, Description = "Server-side session cookie set by POST /auth/login." };
...
app.UseExceptionHandler(handler => handler.Run(async ctx =>
{
    ...
    var (status, code, title) = ex switch
    {
        ...
        DeNoise.Domain.Episodes.InvalidEpisodeTransitionException => (409, "invalid-transition", "Invalid transition"),
...
await using (var scope = app.Services.CreateAsyncScope())
{
    try
    {
        var seededTemplates = await scope.ServiceProvider.GetRequiredService<DeNoise.Application.Notifications.Templates.TemplateService>().EnsureBuiltInsAsync();
