...
/// clients. The resolved <see cref="DeNoisePrincipal"/> is stored in <c>HttpContext.Items</c> for handlers.
/// </summary>
public sealed class DeNoiseAuthenticationHandler(IOptionsMonitor<AuthenticationSchemeOptions> options, ILoggerFactory logger, UrlEncoder encoder)
    : AuthenticationHandler<AuthenticationSchemeOptions>(options, logger, encoder)
{
    public const string SchemeName = "DeNoise";
    public const string CookieName = "denoise_session";
    public const string PrincipalKey = "DeNoise.Principal";

    protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
    {
        DeNoisePrincipal? principal = null;
        ...
        else if (Request.Cookies.TryGetValue(CookieName, out var cookie) && !string.IsNullOrEmpty(cookie))
        {
            ...
            principal = DeNoisePrincipal.ForSession(resolved.Value.User, resolved.Value.Session.SessionId);
        }
...
{
    public static DeNoisePrincipal Principal(this HttpContext http)
        => http.Items[DeNoiseAuthenticationHandler.PrincipalKey] as DeNoisePrincipal ?? throw new InvalidOperationException("no authenticated principal");

    public static DeNoisePrincipal? PrincipalOrNull(this HttpContext http) => http.Items[DeNoiseAuthenticationHandler.PrincipalKey] as DeNoisePrincipal;
