...
public static class AuthEndpoints
{
    public static IEndpointRouteBuilder MapAuth(this IEndpointRouteBuilder app)
    {
        ...
        group.MapPost("/login", async (LoginRequest request, HttpContext http, AuthService auth, IOptions<LocalAuthOptions> options) =>
        {
            ...
            http.Response.Cookies.Append(AlertHubAuthenticationHandler.CookieName, result.SessionToken!, new CookieOptions
            {
                HttpOnly = true,
        ...
        group.MapPost("/logout", async (HttpContext http, AuthService auth) =>
        {
            ...
            if (http.Request.Cookies.TryGetValue(AlertHubAuthenticationHandler.CookieName, out var cookie) && principal is not null && cookie is not null)
            {
