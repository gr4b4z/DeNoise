...
public sealed class SmtpOptions
{
    ...
    public string From { get; set; } = "denoise@example.invalid";
    ...
    public string MessageIdDomain { get; set; } = "denoise.local";
}

/// <summary>
/// SMTP e-mail (06 §7): subject <c>[DeNoise][{severity}] {summary} — {resourceName}</c>, plain-text body, <c>Message-ID</c>
...
public sealed class SmtpEmailChannel(IOptions<SmtpOptions> options, ILogger<SmtpEmailChannel> logger) : INotificationChannel
{
    ...
    public static MimeMessage Compose(OutboxMessage message, string body, string[] recipients, SmtpOptions smtp)
    {
        ...
        mime.Subject = $"[DeNoise][{severity}] {summary}" + (string.IsNullOrEmpty(resource) ? string.Empty : $" — {resource}");
        mime.MessageId = MessageId(message.OutboxId, smtp.MessageIdDomain);
        mime.Headers.Add("X-DeNoise-Delivery-Id", message.OutboxId.ToString());
        mime.Headers.Add("X-DeNoise-Event", message.Type);
    ...
    private static string PlainText(OutboxMessage message, JsonObject model)
    {
        ...
        var episode = model["episode"] as JsonObject;
        sb.Append("DeNoise notification: ").AppendLine(message.Type);
        sb.AppendLine();
