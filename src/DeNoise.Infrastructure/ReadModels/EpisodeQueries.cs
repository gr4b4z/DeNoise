...
/// </summary>
public sealed class EpisodeQueries(DeNoiseDbContext db, NpgsqlDataSource dataSource, TimeProvider time) : IEpisodeQueries
{
    ...
    public async Task<PagedResponse<EpisodeListItem>> ListAsync(DeNoisePrincipal principal, EpisodeFilter filter, IReadOnlyCollection<Guid> myTeamIds, CancellationToken ct = default)
    {
    ...
    public async Task<EpisodeDetail?> GetAsync(DeNoisePrincipal principal, Guid episodeId, CancellationToken ct = default)
    {
        var now = time.GetUtcNow();
    ...
    public async Task<PagedResponse<TimelineEntry>> TimelineAsync(DeNoisePrincipal principal, Guid episodeId, int limit, string? cursor, string? kind, CancellationToken ct = default)
    {
    ...
    public async Task<RelatedEpisodes?> RelatedAsync(DeNoisePrincipal principal, Guid episodeId, CancellationToken ct = default)
    {
    ...
    public async Task<(byte[] Body, string? ContentType, DateTimeOffset ReceivedAt)?> RawPayloadAsync(DeNoisePrincipal principal, Guid episodeId, Guid eventId, CancellationToken ct = default)
    {
    ...
    public async Task<IReadOnlyDictionary<string, int>> ViewCountsAsync(DeNoisePrincipal principal, IReadOnlyCollection<Guid> myTeamIds, CancellationToken ct = default)
    {
        var now = time.GetUtcNow();
        var where = new List<string>();
        var parameters = new List<NpgsqlParameter>();
    ...
    public async Task<TeamOverview?> TeamOverviewAsync(DeNoisePrincipal principal, Guid teamId, CancellationToken ct = default)
    {
    ...
    private static void ScopeClause(DeNoisePrincipal principal, List<string> where, List<NpgsqlParameter> parameters)
    {
