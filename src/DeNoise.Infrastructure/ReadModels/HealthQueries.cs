...
public sealed class HealthQueries(DeNoiseDbContext db, IConfiguration configuration, TimeProvider time)
{
    ...
    public async Task<HubHealth> HubAsync(CancellationToken ct = default)
    {
        ...
        var configured = !string.IsNullOrWhiteSpace(configuration["DeNoise:ExternalDeadmanUrl"]);
