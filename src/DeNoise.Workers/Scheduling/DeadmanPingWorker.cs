...
{
    public const string Section = "DeNoise";
...
/// that does not depend on DeNoise. The last successful ping is recorded as component <c>deadman</c> for the hub health screen.
/// </summary>
public sealed class DeadmanPingWorker(IServiceScopeFactory scopes, IHttpClientFactory httpClients, IOptions<DeadmanOptions> options, TimeProvider time, ILogger<DeadmanPingWorker> logger) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        {
            logger.LogWarning("No DeNoise:ExternalDeadmanUrl configured: DeNoise cannot report its own scheduler failure (spec §13.7 launch blocker)");
            return;
        }
        if (!Uri.TryCreate(url, UriKind.Absolute, out var target) || (target.Scheme != Uri.UriSchemeHttps && target.Scheme != Uri.UriSchemeHttp))
        {
            logger.LogError("DeNoise:ExternalDeadmanUrl '{Url}' is not an absolute http(s) URL; dead-man ping disabled", url);
            return;
        }
    ...
    public async Task<bool> PingOnceAsync(Uri target, CancellationToken ct)
    {
        ...
        var db = scope.ServiceProvider.GetRequiredService<DeNoiseDbContext>();
