...
try
{
    ...
    {
        var teams = scope.ServiceProvider.GetRequiredService<DeNoise.Application.Teams.ITeamRepository>();
        if (await teams.GetTriageAsync() is null)
        {
            await scope.ServiceProvider.GetRequiredService<DeNoise.Application.Teams.TeamService>()
                .CreateAsync(new DeNoise.Application.Teams.CreateTeam("triage", ["dev"], IsTriage: true), Actor.System(Guid.NewGuid().ToString("N"), "migrator:seed-dev"));
