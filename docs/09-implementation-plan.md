...
| **6** | Heartbeats | `/hb` ping API, `hb` schema, state machine (04 §10), cron with tz (DST tests), miss episodes, pause/resume, token rotate, maintenance auto-pause, binding to integration coverage; React: heartbeat list/create/detail with one-time URL dialog and snippets; YAML import/export | *Registered heartbeat misses its schedule*, *misses many intervals*, *Cron heartbeat crosses DST*, *`/fail`*, *Token rotated*, *Paused*, *Maintenance covers heartbeat*, *DeNoise scheduler stops* | none |
