# Alert Hub — Product and Architecture Specification
...
This document defines a bounded first release of Alert Hub: a central operational workspace for receiving, normalising, deduplicating, owning, tracking and delivering alerts from heterogeneous monitoring systems.
...
| **Condition state** | What Alert Hub believes about the monitored system. |
...
| **Registered heartbeat** | A dead-man's switch registered by a team through the UI or API. The producer calls Alert Hub on a schedule; silence past the grace period raises an alert. |
...
| G6 | Operators actually use it | Share of acknowledgements performed in Alert Hub rather than in email or Teams | ≥ 80% of pilot-team alerts after 8 weeks |
...
| Hidden cost | Alert Hub becomes a tier-0 dependency: if it is down, nobody learns that anything else is down |
...
| Hub operator | Runs Alert Hub and handles processing and delivery failures |
...
1. **Alert Hub timers use elapsed time and are business-hours aware only through an explicit policy flag per team.** There is no availability inference.
...
3. **Team → person resolution out of hours is delegated,** not invented: the escalation destination for out-of-hours may be an external system that owns rotations (JSM/Opsgenie-successor, or a phone tree). Alert Hub records that it escalated and to where.
...
| `received_at` | Alert Hub receipt time |
...
### 12.2 Automatic resolution is configured in Alert Hub

Automatic closure is a core Alert Hub capability, configured per rule **inside the Hub**. It is not inherited from the source, does not require the producer to change anything, and is **enabled by default** for every rule. The profile in 12.1 supplies a preset; every field of it is operator-overridable during onboarding.
...
```yaml
# Configured per rule in Alert Hub; the source is not involved.
lifecycle:
...
- Alert Hub processing failed.

Absence of alert traffic proves none of these.

### 13.2 Heartbeat is a first-class Alert Hub capability

Neither Azure Monitor nor Atlas emits a heartbeat through the alert path. Alert Hub therefore **provides** the heartbeat rather than waiting for one. Two mechanisms, deliberately different in what they prove:
...
| Direction | Push: the producer calls Alert Hub on a schedule | Pull-through: a synthetic rule in the source fires through the real alert path |
...
| Alert Hub itself | Internal worker heartbeats, plus an external dead-man's switch on a third-party service (§13.7) |
...
Alert Hub can report source failures only while its own scheduler, database and notification path work. An external system must independently check ingestion health, application API health, database availability, queue lag, scheduler operation and dispatcher operation — over an independent notification path.

**Alert Hub must never be the only mechanism that can report its own failure.** This is a launch blocker, not a nice-to-have, and it is the direct consequence of making Alert Hub a tier-0 dependency.
...
Alert Hub does not carry vendor-specific channels in v1. It has exactly two delivery channels:
...
Everything else is a webhook destination with a template: Microsoft Teams (via a Power Automate Workflows HTTP trigger, since Office 365 connectors were retired in May 2026), Slack, Jira, PagerDuty-style systems, or an automation platform that itself sends email. Alert Hub ships **built-in templates** for the common receivers (generic JSON, Teams Adaptive Card for Workflows, Slack Block Kit, plain text) and lets an administrator write new ones; adding a receiver never requires a code release.
...
- Every request carries `X-AlertHub-Delivery-Id` (stable across retries) and an HMAC signature so receivers can verify and deduplicate.
...
| Alert Hub scheduler stops | External dead-man's switch reports it; missed heartbeats are not silently reported as healthy |
...
| 4 | Which team owns Alert Hub in steady state, and who is on call for it? | CTO | Before the gate |
