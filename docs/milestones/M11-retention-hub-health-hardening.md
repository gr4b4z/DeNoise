...
| Hub API | `HubApiTests` — health for operators, failures 403 / 200 by role, retry of an unknown id 404, retention status (30 d, partitions, never ran, next run), manual run 403 / 200 with the report, status then shows the run, audit lists `retention.run` by *Alert Hub retention*. |
