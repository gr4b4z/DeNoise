...
| Area | Where | Notes |
|---|---|---|
| Solution layout | `DeNoise.sln`, `src/`, `tests/` | Layout from AGENTS.md §3. `Domain` has no package references. Central package versions in `Directory.Packages.props`; warnings are errors. |
| Hosts | `src/DeNoise.Api`, `src/DeNoise.Ingest`, `src/DeNoise.Workers` | Minimal APIs; each exposes `/healthz/live|ready|startup`. Workers take `--roles=processing,scheduler,dispatcher`. |
| Migrator | `src/DeNoise.Migrator` | Applies EF migrations, then ensures `raw_event` partitions. Runs as the Helm pre-upgrade Job (ADR-10). Exit code 1 on failure. |
| First migration | `src/DeNoise.Infrastructure/Persistence/Migrations/*_InitialSchema.cs` | Schemas `alert`, `ops`, `audit`, `hb`, `cfg`; **partitioned** `alert.raw_event` (hand-written SQL, daily partitions); `ops.job` with the ★ one-live-timer-per-episode index; `ops.outbox`; `ops.hub_component_heartbeat`; `fillfactor=70` + aggressive autovacuum on the queue tables. |
...
| Observability | `Infrastructure/Observability` | Serilog console JSON; OpenTelemetry traces + metrics with OTLP when `Otel:Endpoint` is set; `DeNoiseMetrics` owns the ADR-9 instrument names. |
...
| Helm | `deploy/helm/denoise` | Deployments + Services + PDBs for the three hosts, migrate Job hook, optional Ingress, all values from 03 §4. |
