...
| Webhook channel | `WebhookChannel` | ADR-7 contract: `POST`, `User-Agent`, `X-DeNoise-Delivery-Id` (stable across retries), `X-DeNoise-Event`, `X-DeNoise-Timestamp`, `X-DeNoise-Signature: v1=hex(hmac_sha256(secret, ts + "." + body))`, destination headers, per-destination timeout, no redirects, https required unless `AllowInsecureDestinations`; 2xx success, 408/425/429/5xx retryable, other 4xx/3xx permanent, timeout ⇒ `response_lost`; first 4 KiB of the response kept. |
| SMTP channel | `SmtpEmailChannel` | MailKit; subject `[DeNoise][severity] summary — resource`; plain text only; `Message-Id` from the outbox id, `References` to the episode id for threading; `Smtp:*` options (Mailpit: `Security=None`). |
