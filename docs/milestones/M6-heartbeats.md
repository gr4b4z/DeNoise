...
| DeNoise scheduler stops | covered by the milestone-5 dead-man ping and `/hub/health` `deadman` status; the external switch, not the Hub, reports the stop (spec §13.7). |
...
- **Ping URLs come from `Heartbeats:IngestPublicBaseUrl`**, falling back to the Helm-provided `DeNoise:IngestPublicBaseUrl`; the same fallback now applies to `Notifications:PublicBaseUrl` ← `DeNoise:PublicBaseUrl`.

## Configuration

```
Heartbeats:IngestPublicBaseUrl   http://localhost:8081   # or DeNoise:IngestPublicBaseUrl (Helm ingestPublicBaseUrl)
