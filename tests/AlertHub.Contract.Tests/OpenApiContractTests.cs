...
/// security rules; they run without a database. Regenerate the snapshot with <c>UPDATE_OPENAPI=1 dotnet test tests/AlertHub.Contract.Tests</c>.
/// </summary>
[Trait("Category", "Unit")]
public sealed class OpenApiContractTests : IAsyncLifetime
{
    private WebApplicationFactory<AlertHub.Api.ApiHost> _factory = null!;
    ...
    {
        _factory = new WebApplicationFactory<AlertHub.Api.ApiHost>().WithWebHostBuilder(b =>
    ...
    public void Document_is_openapi_3_and_names_the_api()
    {
        ...
        _document.RootElement.GetProperty("info").GetProperty("title").GetString().Should().Be("Alert Hub application API");
    }
    ...
    public void Committed_snapshot_matches_the_running_api()
    {
        ...
        committed.Should().Be(pretty, "docs/openapi/v1.json is out of date; run UPDATE_OPENAPI=1 dotnet test tests/AlertHub.Contract.Tests and commit the result");
    }

    private static string FindRepoRoot()
    {
        var dir = new DirectoryInfo(AppContext.BaseDirectory);
        while (dir is not null && !File.Exists(Path.Combine(dir.FullName, "AlertHub.sln"))) dir = dir.Parent;
