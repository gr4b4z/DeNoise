...
using TemplateRenderer = DeNoise.Application.Notifications.Templates.TemplateRenderer;

namespace DeNoise.Application.Tests.Notifications;
...
public sealed class TemplateRendererTests
{
    ...
    {
        var errors = new List<DeNoise.Application.Mapping.MappingValidationError>();
        ...
        foreach (var template in BuiltInTemplates.Create(T0))
        {
            ...
            if (template.Format == TemplateFormats.Json)
            {
                ...
                if (template.TemplateId == BuiltInTemplates.TeamsAdaptiveCard)
                {
                    ...
                    card.GetProperty("actions")[0].GetProperty("url").GetString().Should().StartWith("https://denoise.example.invalid/episodes/");
                }
            ...
            {
                rendered.Body.Should().StartWith("[DeNoise][critical] Disk \"C:\\\" full");
            }
        }
    }
}
