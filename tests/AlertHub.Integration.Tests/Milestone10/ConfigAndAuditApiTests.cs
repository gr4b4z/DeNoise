...
namespace AlertHub.Integration.Tests.Milestone10;
...
public sealed class ConfigAndAuditApiTests(PostgresFixture postgres) : IAsyncLifetime
{
    ...
    public async Task Export_import_round_trip_is_unchanged_and_edits_create_new_active_versions()
    {
        ...
        yaml.Should().Contain("alerthub_config: 1").And.Contain("routing:").And.Contain("escalation:").And.Contain("lifecycle:").And.Contain("grouping:")
        ...
        var (bs, bad, braw) = await admin.PostAsync<ConfigImportResponse>("/api/v1/config/import", new ConfigImportRequest("alerthub_config: 1\npolicies:\n  weather: []\n  lifecycle:\n    - document: { lifecycle: { profile: repeating_while_active } }\n", DryRun: true));
