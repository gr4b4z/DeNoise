...
public sealed class ChannelTests : IAsyncLifetime
{
    ...
    public async Task Webhook_sends_the_contract_headers_and_a_verifiable_signature()
    {
        ...
        req.Headers["X-AlertHub-Delivery-Id"].Should().Be(message.OutboxId.ToString());
        req.Headers["X-AlertHub-Event"].Should().Be(NotificationTypes.EpisodeOpened);
        req.Headers["X-Custom"].Should().Be("yes");
        req.Headers["User-Agent"].Should().StartWith("AlertHub/");
        req.Headers["Content-Type"].Should().StartWith("application/json");
        var timestamp = req.Headers["X-AlertHub-Timestamp"];
        ...
        req.Headers["X-AlertHub-Signature"].Should().Be(expected, "receiver-side recompute over timestamp.body matches");
    ...
    public async Task Smtp_channel_sends_a_threaded_plain_text_mail_with_a_stable_message_id()
    {
        ...
        mail.Should().Contain("Subject: [AlertHub][critical] Disk <b>full</b>").And.Contain("db-1");
