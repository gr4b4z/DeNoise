...
public sealed class ApiFixture : IAsyncDisposable
{
    ...
    public HostFactory<AlertHub.Api.ApiHost> Factory { get; private set; } = null!;
    ...
    public static async Task<ApiFixture> CreateAsync(PostgresFixture postgres, string name, IDictionary<string, string?>? settings = null)
    {
        ...
        f.Factory = new HostFactory<AlertHub.Api.ApiHost>(f.ConnectionString, f.Time, settings: all);
        ...
        var adminPrincipal = AlertHubPrincipal.ForSession(admin, "setup");
