...
public sealed class ApiClient : IDisposable
{
    ...
    public async Task<HttpResponseMessage> LoginAsync(string username, string password)
    {
        ...
        {
            var setCookie = response.Headers.GetValues("Set-Cookie").First(c => c.StartsWith("alerthub_session=", StringComparison.Ordinal));
            _cookie = setCookie.Split(';')[0]["alerthub_session=".Length..];
            _bearer = null;
    ...
    private async Task<HttpResponseMessage> SendAsync(HttpMethod method, string path, object? body, int? ifMatch = null, Guid? idempotencyKey = null, bool csrf = true, bool idempotency = true)
    {
        ...
        else if (_cookie is not null) request.Headers.Add("Cookie", $"alerthub_session={_cookie}");
