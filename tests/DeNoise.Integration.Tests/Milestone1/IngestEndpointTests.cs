...
namespace DeNoise.Integration.Tests.Milestone1;
...
public sealed class IngestEndpointTests(PostgresFixture postgres) : IAsyncLifetime
{
    ...
    private HostFactory<DeNoise.Ingest.IngestHost> _factory = null!;
    private IntegrationCredentials _credentials = null!;

    public async Task InitializeAsync()
    {
        _cs = await postgres.CreateDatabaseAsync("ingest");
        _factory = new HostFactory<DeNoise.Ingest.IngestHost>(_cs, settings: new Dictionary<string, string?>
        {
    ...
    public async Task Valid_token_returns_202_and_commits_raw_event_with_normalise_job()
    {
        ...
        request.Headers.TryAddWithoutValidation("X-DeNoise-Source-Time", "2026-09-11T10:00:00Z");
        ...
        raw.Headers.Should().Contain("X-DeNoise-Source-Time").And.NotContain("X-Secret-Header");
    ...
    public async Task Database_unavailable_returns_503_not_202()
    {
        ...
        await using var factory = new HostFactory<DeNoise.Ingest.IngestHost>(bad);
