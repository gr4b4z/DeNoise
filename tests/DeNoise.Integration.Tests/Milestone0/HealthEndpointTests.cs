...
namespace DeNoise.Integration.Tests.Milestone0;
...
public sealed class HealthEndpointTests(PostgresFixture postgres) : IAsyncLifetime
{
    ...
    {
        await using var factory = new HostFactory<DeNoise.Api.ApiHost>(_cs);
    ...
    public async Task Ready_is_503_when_database_is_unreachable()
    {
        ...
        await using var factory = new HostFactory<DeNoise.Api.ApiHost>(bad);
        using var client = factory.CreateClient();
