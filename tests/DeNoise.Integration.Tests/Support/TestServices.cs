...
public static class TestServices
{
    public static ServiceProvider Build(string connectionString, TimeProvider? time = null, Action<IServiceCollection>? configure = null, IDictionary<string, string?>? settings = null)
    {
        ...
        services.AddSingleton<DeNoiseMetrics>();
        services.AddDeNoisePersistence(connectionString);
        services.AddDeNoiseInfrastructure(configuration);
    ...
    public static Task<T> InDbAsync<T>(IServiceProvider provider, Func<DeNoiseDbContext, Task<T>> action)
        => InScopeAsync(provider, sp => action(sp.GetRequiredService<DeNoiseDbContext>()));
}
