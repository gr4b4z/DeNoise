...
public sealed class HeartbeatPingEndpointTests(PostgresFixture postgres) : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        _factory = new HostFactory<DeNoise.Ingest.IngestHost>(_cs, new FakeTimeProvider(new DateTimeOffset(2026, 9, 11, 12, 0, 0, TimeSpan.Zero)), settings: new Dictionary<string, string?>
        {
        ...
        var principal = DeNoisePrincipal.ForSession(new User { UserId = Guid.NewGuid(), Username = "op", DisplayName = "Op", RoleNames = [Roles.Operator], Scopes = ["scope-a"] }, "s");
