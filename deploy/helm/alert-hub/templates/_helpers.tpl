{{- define "alert-hub.name" -}}{{ .Chart.Name }}{{- end -}}
{{- define "alert-hub.fullname" -}}{{ printf "%s" .Release.Name | trunc 63 | trimSuffix "-" }}{{- end -}}
{{- define "alert-hub.tag" -}}{{ .Values.image.tag | default .Chart.AppVersion }}{{- end -}}
{{- define "alert-hub.labels" -}}
app.kubernetes.io/name: {{ include "alert-hub.name" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
app.kubernetes.io/version: {{ include "alert-hub.tag" . | quote }}
app.kubernetes.io/managed-by: {{ .Release.Service }}
helm.sh/chart: {{ .Chart.Name }}-{{ .Chart.Version }}
{{- end -}}
{{- define "alert-hub.selectorLabels" -}}
app.kubernetes.io/name: {{ include "alert-hub.name" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
{{- end -}}
{{/* Environment shared by every host container */}}
{{- define "alert-hub.env" -}}
- name: ConnectionStrings__AlertHub
  valueFrom:
    secretKeyRef:
...
- name: AlertHub__PublicBaseUrl
  value: {{ .Values.publicBaseUrl | quote }}
- name: AlertHub__IngestPublicBaseUrl
  value: {{ .Values.ingestPublicBaseUrl | quote }}
- name: AlertHub__ExternalDeadmanUrl
...
{{- define "alert-hub.deployment" -}}
{{- $root := .root -}}{{- $host := .host -}}{{- $spec := .spec -}}
apiVersion: apps/v1
kind: Deployment
metadata:
  name: {{ include "alert-hub.fullname" $root }}-{{ $host }}
  labels:
    {{- include "alert-hub.labels" $root | nindent 4 }}
    app.kubernetes.io/component: {{ $host }}
spec:
  replicas: {{ $spec.replicas }}
  selector:
    matchLabels:
      {{- include "alert-hub.selectorLabels" $root | nindent 6 }}
      app.kubernetes.io/component: {{ $host }}
  template:
    metadata:
      labels:
        {{- include "alert-hub.selectorLabels" $root | nindent 8 }}
        app.kubernetes.io/component: {{ $host }}
    spec:
          ...
          preferredDuringSchedulingIgnoredDuringExecution:
            - weight: 100
              podAffinityTerm:
                ...
                labelSelector:
                  matchLabels:
                    {{- include "alert-hub.selectorLabels" $root | nindent 20 }}
                    app.kubernetes.io/component: {{ $host }}
      containers:
        - name: {{ $host }}
          image: "{{ $root.Values.image.repository }}-{{ $host }}:{{ include "alert-hub.tag" $root }}"
          ...
          ports:
            - name: http
              containerPort: 8080
          env:
            {{- include "alert-hub.env" $root | nindent 12 }}
          startupProbe:
